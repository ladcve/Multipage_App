...
import pandas as pd
from library import listToString
...
class gascondensate():
    ...
    def plot(self, p, z, Gp, F, Btg, Efw, Eg, Rv):
        ...
        fig1.add_trace(go.Scatter(x=x1, y=y1, mode='lines'))
        fig1.update_layout(title='Gráfico 1: F vs Eg',
                   xaxis_title='Eg (RB/scf)',
                   yaxis_title='F (res ft3)')
        ...
        Nfoi = 0
        labels, handles, OOIP, OGIP = initial_hydrocarbon_in_place(Nfoi, Gfgi, Rv, Rs)
        str_labels1 = listToString(labels) 
        ...
        fig1.add_traces(go.Scatter(x=x1_fit, y=y1_fit, mode='lines', name=labels[3]))
        ...
        if np.all(z==0) == False:        
            ...
            fig2.update_layout(title='Gráfico 2: p/z vs Gp',
                   xaxis_title='Gp (scf)',
            ...
            Nfoi = 0
            labels, handles, OOIP, OGIP = initial_hydrocarbon_in_place(Nfoi, Gfgi, Rv, Rs)
            str_labels2 = listToString(labels) 

            ## plot the regression line
            x2_fit = np.linspace(min(x2), max(x2), 5)
            y2_fit = linear_with_intercept(x2_fit, m, c)
            #plt.plot(x2_fit, y2_fit)
            fig2.add_traces(go.Scatter(x=x2_fit, y=y2_fit, mode='lines', name=labels[3]))
            #fig2.add_trace(px.line(df2_fit, x='x2_fit', y='y2_fit'))
        ...
        x3, y3 = Gp, (F / Eg)
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(x=x3, y=y3, mode='lines'))

        fig3.update_layout(title='Gráfico Waterdrive Diagnostic',
                   xaxis_title='Gp (scf)',
                   yaxis_title='F/Eg (scf)')
        # plt.plot(x3, y3, '.-')
        # plt.title('Plot 3: Waterdrive Diagnostic Plot', size=title_size, pad=title_pad)
        # plt.xlabel('Gp (scf)')
        # plt.ylabel('F/Eg (scf)')
        ...
        Nfoi = 0
        labels, handles, OOIP, OGIP = initial_hydrocarbon_in_place(Nfoi, Gfgi, Rv, Rs)
        str_labels3 = listToString(labels)     

        ## plot the regression line
        x3_fit = np.linspace(min(x3[1:]), max(x3[1:]), 5)
        y3_fit = linear_with_intercept(x3_fit, m, Gfgi)
        #plt.plot(x3_fit, y3_fit)
        fig3.add_traces(go.Scatter(x=x3_fit, y=y3_fit, mode='lines', name=labels[3]))
        #fig3.add_trace(px.line(df3_fit, x='x3_fit', y='y3_fit'))
        ...
        x6, y6 = (Eg + Bgi * Efw), F
        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(x=x6, y=y6, mode='lines'))
        fig4.update_layout(title='Gráfico 4: F vs (Eg+Bgi*Efw)',
                   xaxis_title='Eg+Bgi*Efw (res ft3/scf)',
                   yaxis_title='F (res ft3)')
        # plt.plot(x6, y6, '.-')
        # plt.title('Plot 6: F vs (Eg+Bgi*Efw)', size=title_size, pad=title_pad)
        # plt.xlabel('Eg+Bgi*Efw (res ft3/scf)')
        ...
        Nfoi = 0
        labels, handles, OOIP, OGIP = initial_hydrocarbon_in_place(Nfoi, Gfgi, Rv, Rs)
        str_labels4 = listToString(labels)  

        ## plot the regression line
        x6_fit = np.linspace(min(x6), max(x6), 5)
        y6_fit = linear_zero_intercept(x6_fit, Gfgi)
        #plt.plot(x6_fit, y6_fit)
        fig4.add_traces(go.Scatter(x=x6_fit, y=y6_fit, mode='lines', name=labels[3]))
        #fig4.add_trace(px.line(df6_fit, x='x6_fit', y='y6_fit'))
        ...
        if np.all(z==0) == False:
            ...
            #plt.plot(x7, y7, '.-')
            fig5 = go.Figure()
            fig5.add_trace(go.Scatter(x=x7, y=y7, mode='lines'))
            fig5.update_layout(title='Gráfico 5: ((p/z)*(1-Efw)) vs Gp',
                   xaxis_title='Gp (scf)',
                   yaxis_title='(p/z)*(1-Efw) (psia)')
            ## curve-fitting to calculate the slope as OGIP
            x7_norm = x7 / max(x7) # normalize x
            y7_norm = y7 / max(y7) # normalize y
            popt, pcov = curve_fit(linear_with_intercept, x7_norm, y7_norm)
            ...
            Nfoi = 0
            labels, handles, OOIP, OGIP = initial_hydrocarbon_in_place(Nfoi, Gfgi, Rv, Rs)
            str_labels5 = listToString(labels)          

            ## plot the regression line
            x7_fit = np.linspace(min(x7), max(x7), 5)
            y7_fit = linear_with_intercept(x7_fit, m, c)
            #plt.plot(x7_fit, y7_fit)
            fig5.add_traces(go.Scatter(x=x7_fit, y=y7_fit, mode='lines', name=labels[3]))
            #fig5.add_trace(px.line(df7_fit, x='x7_fit', y='y7_fit'))
        ...
        #return F, Eg, Efw
        return fig1,fig2,fig3,fig4,fig5,str_labels1,str_labels2,str_labels3,str_labels4,str_labels5
