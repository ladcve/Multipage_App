...
layout = html.Div([
    dbc.Tabs([
                    ...
                    dbc.Card([
                        ...
                        dbc.Row([
                            ...
                            dbc.Col([
                                ...
                                id="btn_save_file_edit", color="primary", n_clicks=0, className="mr-1"),
                ...
                dbc.Col([
                    dbc.Row([
                        dbc.Button(html.Span(["ejecutar ", html.I(className="fas fa-running ml-1")],style={'font-size':'1.5em','text-align':'center'}),
                         id="btn_ejecutar_query_edit", color="success", n_clicks=0, className="mr-2"),
                    ]),
                    dbc.Row([
                        dac.Box([
                                dac.BoxHeader(
        ...
        dbc.Tab([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Input(id="inp-ruta-sql", placeholder="Type something...", type="text", style={'backgroundColor':'white'}),
                    html.Div(id="loading-sql-message")
                ], width=4),
                dbc.Col([
                    dbc.Button(html.Span(["salvar ", html.I(className="fas fa-save ml-1")],style={'font-size':'1.5em','text-align':'center'}),
                     id="btn_save_file", color="primary", n_clicks=0, className="mr-1"),
            ...
            with open(os.path.join(QUERY_DIRECTORY, archivo)) as f:
                ...
                if contenido:
                    for linea in contenido:
                        sentencia_SQL +=  linea 
    return archivo, sentencia_SQL

@app.callback(
    [Output("dt_query_results_edit", "data"), Output("dt_query_results_edit", "columns")],
    [Input("btn_ejecutar_query_edit", "n_clicks"),
     Input('textarea-edit', 'value') ],
    [State('dt_query_results_edit', 'data'), State('dt_query_results_edit', 'columns')]
)
def update_sqlresults(n_clicks, sql_text, data, columns):
    df = pd.DataFrame()
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]

    if 'btn_ejecutar_query_edit' in changed_id:
        con = sqlite3.connect(archivo)
        if sql_text:
